...
from .context import timestamp_tools
...
def test_parse(stem, Y, m, d, H, M, S, ext):
    ...
    if valid:
        ctime = timestamp_tools.parse_ctime(string)
    else:
        with pytest.raises(AssertionError):
            timestamp_tools.parse_ctime(string)
...
#     string = timestamp_tools.format_ctime(ctime)
#     ctime2 = timestamp_tools.parse_ctime(string)
...
def test_format(ctime):
    string = timestamp_tools.format(ctime)
    ctime2 = timestamp_tools.parse(string)
...
def test_out_of_date(reference, target):
    outdated = timestamp_tools.out_of_date(reference, target)
    assert outdated in (True, False, None)
    # assert (outdated is None and outdated2 is None) or (outdated != outdated2)
    if outdated:
        outdated2 = timestamp_tools.out_of_date(target, reference)
